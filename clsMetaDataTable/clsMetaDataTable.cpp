...
clsMetaDataTable::clsMetaDataTable(int x, int y, int w, int h, const char *l):Fl_Table_Row(x,y,w,h,l){
   ...
   type(SELECT_SINGLE);
  //  when(FL_WHEN_ENTER_KEY_ALWAYS|FL_WHEN_NOT_CHANGED|when());
...
void clsMetaDataTable::draw_cell(TableContext context, int ROW, int COL, int X, int Y, int W, int H) {
    	...
    	case CONTEXT_RC_RESIZE:
	{
      ...
      case CONTEXT_CELL:  // Draw data in cells
          {
          if (   (context_edit == context)
              && (ROW == row_edit)
              && (COL == col_edit)
              && (txtin_sl->visible()))
            return;
          bool swcRowOK=ROW<0||static_cast<size_t>(ROW)<Data->size();
        switch(COL){
         case 0: if(swcRowOK){
...
//----  --------------------------------------------------------------------------------------------------------
 int clsMetaDataTable::handle(int e) {
    ...
    if(e == FL_MOUSEWHEEL){
     ...
     //  printf(" if e FL_KEYBOARD: %d %d %s.\n",e,Fl::event_key(),Fl::event_text());
 ...
 //  printf("ret: %d %d %s.\n",ret,Fl::event_key(),Fl::event_text());
   return ret;
};
